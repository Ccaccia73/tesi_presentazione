...
\begin{frame}{Adapter Input}
  ...
  \item[preCICE] \textcolor{pblue}{config file, participant name, ...}
  \item[Simulation] \textcolor{pblue}{mesh location} (used for coupling and mapping)\\
                    \textcolor{pblue}{data to pass} (displacements or deltas)\\
                    \textcolor{pblue}{starting ramp configuration} (type, period, ...)
  \item[Output]     \textcolor{pblue}{vtk file} (location, name, interval, ...)\\
                    \textcolor{pblue}{resultants} (name, origin for moments, ...)
  \end{description}
