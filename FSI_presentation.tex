...
\begin{itemize}
    \item Exploit \textbf{MBDyn} capabilities to perform fluid-structure co-simulation
    \item Integrate \textit{MBDyn} with \textbf{preCICE} to guarantee a common interface to multiple CFD solvers
    \item validate the \textbf{FSI} simulation with the proposed setup
\end{itemize}
...
\begin{itemize}
    \item the \textcolor{dblue}{\textbf{adapter}} has been developed and verified
    \item Solutions comparable with benchmarks (bluff body, FSI2...)
\end{itemize}
...
\begin{itemize}
    \item Convergence issues when \textit{mass ratio} $\rightarrow 1$ 
\end{itemize}
...
Three possible directions of further development:
